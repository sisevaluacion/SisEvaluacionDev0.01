...
namespace MvcPlantilla.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //static SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString());
        //public static bool ValidarUsuario(string usuario, string contrasena)
        //{
        //    bool Autenticacion = false;
        //    string Consulta = string.Format("Select * from [TUsuario] where Usuario = '{0}' and Contrasena = '{1}'", usuario, contrasena);
        //    SqlCommand cmd = new SqlCommand(Consulta, con);
        //    con.Open();
        //    SqlDataReader sdr = cmd.ExecuteReader();
        //    Autenticacion = sdr.HasRows;
        //    con.Close();
        ...
        BDSisEvaluacionDataContext oBDSisEvaluacion = new BDSisEvaluacionDataContext();
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            {
                var Consulta = from C in oBDSisEvaluacion.uspVerificarUsuario(model.UserName,model.Password) select C;
                byte CodError = 0;
        ...
        public ActionResult Manage(LocalPasswordModel model)
        {
            if (ModelState.IsValid)
            {
                var Consulta = from C in oBDSisEvaluacion.uspActualizarContrasena(User.Identity.Name, model.OldPassword, model.NewPassword) select C;
                byte CodError = 0;
                foreach (var x in Consulta)
                {
                    CodError = Convert.ToByte(x.CodError);
                ...
                if (CodError == 0)
                {
                    //ModelState.AddModelError("", "La contraseña se actualizo correctamente.");
                    return RedirectToAction("Index", "Home");
                }
                else
                {
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    //return RedirectToAction("Index", "Home");
                    var Consulta = from C in oBDSisEvaluacion.uspAgregarUsuario(model.UserName,model.Password) select C;
                    byte CodError = 0;
                    foreach (var x in Consulta)
                    {
                        CodError = Convert.ToByte(x.CodError);
                    ...
                    if (CodError == 0)
                    {
                        var Consulta1 = from C in oBDSisEvaluacion.uspVerificarUsuario(model.UserName, model.Password) select C;
                        FormsAuthentication.SetAuthCookie(model.UserName, false);
                        return RedirectToAction("Index", "Home");
                    }
                    else
                    {
                        ModelState.AddModelError("", "Error : Ud ya Esta Registrado");
                    }
                }
        ...
        private void Login(RegisterModel model, string p)
        {
